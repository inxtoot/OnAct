...
from updater import check_update   # 导入更新函数

class RecorderUI:
    def __init__(self, root):
        self.root = root
        self.root.title("OnAct v1.1.8 - 键鼠录播机")   # 版本号已同步为 v1.1.8
        ...
        self.log("OnAct v1.1.8 已启动。使用 F9 录制/停止，F10 回放选中脚本，F11 查看日志，ESC 停止回放/退出。")
    ...
    def _build_ui(self):
        ...
        # 新增：手动检查更新
        help_menu.add_command(label="检查更新", command=lambda: check_update(show_if_no_update=True))

        # 定时任务菜单
        dev_menu = tk.Menu(menubar, tearoff=0)
        menubar.add_cascade(label="定时任务", menu=dev_menu)
        dev_menu.add_command(label="设置定时任务", command=self.show_scheduled_task)

        menubar.add_command(label="bug反馈", command=self.show_bug_feedback)
        ...
        self.record_btn = ttk.Button(control_frame1, text="录制", width=8, command=lambda: self.core.toggle_recording(hide=False))
        self.record_btn.grid(row=0, column=2, padx=2)
        self.playback_btn = ttk.Button(control_frame1, text="回放", width=8, command=self.start_playback_from_selected)
        self.playback_btn.grid(row=0, column=3, padx=2)
        self.stop_playback_btn = ttk.Button(control_frame1, text="停止", width=6, command=self.core.stop_playback, state=tk.DISABLED)
        ...
        for rec in self.recordings:
            if isinstance(rec, dict) and 'id' in rec:
                self.tree.insert('', tk.END, iid=str(rec['id']), values=(0, rec.get('name', '未知'), rec.get('duration_str', '0'), rec.get('timestamp', '')[11:19] if rec.get('timestamp') else ''))
        self.refresh_row_numbers()
        if self.recordings:
            valid_ids = [r['id'] for r in self.recordings if isinstance(r, dict) and 'id' in r]
            self.next_id = max(valid_ids) + 1 if valid_ids else 1

    # -------------------- 定时任务设置 --------------------
    def show_scheduled_task(self):
        messagebox.showinfo("提示", "定时任务功能正在开发中，敬请期待。")
        ...
        win.title("设置定时任务（开发中...）")
        win.geometry("340x350")
        win.resizable(False, False)
        win.attributes('-topmost', True)

        row = 0
        tk.Label(win, text="执行频率:").grid(row=row, column=0, padx=10, pady=5, sticky=tk.W)
        freq_map = {'daily':'每天', 'weekly':'每周', 'monthly':'每月'}
        freq_var = tk.StringVar(value=freq_map.get(freq, '每天'))
        freq_combo = ttk.Combobox(win, textvariable=freq_var, values=['每天', '每周', '每月'], state='readonly', width=8)
        freq_combo.grid(row=row, column=1, padx=10, pady=5, sticky=tk.W)

        row += 1
        tk.Label(win, text="执行时间:").grid(row=row, column=0, padx=10, pady=5, sticky=tk.W)
        time_frame = ttk.Frame(win)
        time_frame.grid(row=row, column=1, padx=10, pady=5, sticky=tk.W)
        hour_var = tk.IntVar(value=hour)
        minute_var = tk.IntVar(value=minute)
        ttk.Spinbox(time_frame, from_=0, to=23, textvariable=hour_var, width=3).pack(side=tk.LEFT)
        tk.Label(time_frame, text=":").pack(side=tk.LEFT)
        ttk.Spinbox(time_frame, from_=0, to=59, textvariable=minute_var, width=3).pack(side=tk.LEFT)

        row += 1
        weekday_frame = ttk.Frame(win)
        weekday_frame.grid(row=row, column=0, columnspan=2, pady=5)
        tk.Label(weekday_frame, text="星期几:").pack(side=tk.LEFT, padx=5)
        weekdays = ['周一', '周二', '周三', '周四', '周五', '周六', '周日']
        weekday_var = tk.StringVar(value=weekdays[weekday] if 0 <= weekday < 7 else '周一')
        weekday_combo = ttk.Combobox(weekday_frame, textvariable=weekday_var, values=weekdays, state='readonly', width=5)
        weekday_combo.pack(side=tk.LEFT, padx=5)

        row += 1
        day_frame = ttk.Frame(win)
        day_frame.grid(row=row, column=0, columnspan=2, pady=5)
        tk.Label(day_frame, text="每月几号:").pack(side=tk.LEFT, padx=5)
        day_var = tk.IntVar(value=day_of_month)
        ttk.Spinbox(day_frame, from_=1, to=31, textvariable=day_var, width=5).pack(side=tk.LEFT, padx=5)

        row += 1
        tk.Label(win, text="循环次数:").grid(row=row, column=0, padx=10, pady=5, sticky=tk.W)
        count_var = tk.IntVar(value=count)
        ttk.Spinbox(win, from_=1, to=100, textvariable=count_var, width=5).grid(row=row, column=1, padx=10, pady=5, sticky=tk.W)

        def toggle_extra_fields(*args):
            selected = freq_var.get()
            weekday_frame.grid_remove()
            day_frame.grid_remove()
            if selected == '每周':
                weekday_frame.grid()
            elif selected == '每月':
                day_frame.grid()
        freq_var.trace('w', toggle_extra_fields)
        toggle_extra_fields()

        def save_schedule():
            freq_map_rev = {'每天': 'daily', '每周': 'weekly', '每月': 'monthly'}
            new_freq = freq_map_rev.get(freq_var.get(), 'daily')
            settings = load_settings(self.settings_file)
            schedule_data = {
                'frequency': new_freq,
                'hour': hour_var.get(),
            ...
            if new_freq == 'weekly':
                try:
                    weekday_idx = weekdays.index(weekday_var.get())
                except ValueError:
                    weekday_idx = 0
                schedule_data['weekday'] = weekday_idx
                extra = f"，星期{weekday_var.get()}"
            elif new_freq == 'monthly':
                schedule_data['day_of_month'] = day_var.get()
                extra = f"，{day_var.get()}号"
            else:
                extra = ""
            settings['schedule'] = schedule_data
            save_settings(settings, self.settings_file)
            self.log(f"定时任务已设置：频率={freq_var.get()}，时间={hour_var.get():02d}:{minute_var.get():02d}{extra}，循环{count_var.get()}次（开发中...）")
            win.destroy()

        btn_frame = ttk.Frame(win)
        btn_frame.grid(row=row+1, column=0, columnspan=2, pady=20)
        ttk.Button(btn_frame, text="保存", command=save_schedule).pack(side=tk.LEFT, padx=10)
        ttk.Button(btn_frame, text="取消", command=win.destroy).pack(side=tk.LEFT, padx=10)

    # -------------------- 菜单功能 --------------------
    def show_bug_feedback(self):
        messagebox.showinfo("Bug反馈", "如果您在使用过程中发现任何问题，请将详细描述发送至：\n\nadmin@onno1.com\n\n感谢您的支持与反馈！")

    def show_about(self):
        about_text = (
            "OnAct v1.1.8 - 键鼠录播机\n版权归属：onno1.com\n\n"
            "本软件使用 MIT 许可证开源。\n允许自由使用、修改、分发和商业销售，\n但必须保留本版权声明和许可声明。\n\n"
            "⚠️ 重要声明：\n根据 MIT 许可证，任何人可自由使用、修改、分发本软件，\n但必须保留原始版权信息。若您对本软件进行二次开发或\n商业打包分发，请勿移除或篡改版权声明，否则可能构成侵权。\n\n"
    ...
    def export_selected(self):
        ...
        file_path = filedialog.asksaveasfilename(defaultextension=".json", filetypes=[("JSON files", "*.json"), ("All files", "*.*")], title="保存录制文件", initialfile=recording.get('name', '录制') + ".json", initialdir=data_dir)
        if not file_path:
            return
        try:
            import json
    ...
    def import_recording(self):
        ...
        if not os.path.exists(data_dir):
            os.makedirs(data_dir)
        file_path = filedialog.askopenfilename(filetypes=[("JSON files", "*.json"), ("All files", "*.*")], title="选择录制文件", initialdir=data_dir)
        if not file_path:
            return
        try:
            import json
            ...
            recording = {'id': self.next_id, 'name': name, 'duration': duration, 'duration_str': duration_str, 'timestamp': timestamp, 'events': events}
    ...
    def add_recording_to_table(self, recording):
        def _add():
            self.tree.insert('', tk.END, iid=str(recording['id']), values=(0, recording['name'], recording['duration_str'], recording['timestamp'][11:19]))
            self.refresh_row_numbers()
    ...
    def on_recording_stopped(self, events, start_time):
        ...
        recording = {'id': self.next_id, 'name': name, 'duration': last_time, 'duration_str': duration_str, 'timestamp': timestamp, 'events': events}
        self.recordings.append(recording)
        self.add_recording_to_table(recording)
        self.next_id += 1
        self.save_recordings()
    ...
    def set_recording_button_state(self, is_recording):
        self.record_btn.config(text="停止" if is_recording else "录制")
    ...
    def save_settings(self):
        settings = {'sample_enabled': self.sample_enabled.get(), 'sample_interval': self.sample_interval.get(), 'precision_mode': self.precision_mode.get(), 'max_log_size_kb': self.max_log_size_kb}
        save_settings(settings, self.settings_file)
